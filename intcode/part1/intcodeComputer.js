...
function runProgram(input, opcodeInput) {
    ...
    while (isRunning && notPaused) {
        ...
        let instruction = parseInstruction(Number(input[i]));
        // The instruction array contains both the intcode as well as the parameter modes
        switch (instruction[0]) {
            case 1: // Summation opcode
                let sum1 = parseParameter(1, input, i, instruction[1], relativeBase);
                let sum2 = parseParameter(2, input, i, instruction[2], relativeBase);
                if (instruction[3] === 0) {
            ...
            case 2: // Multiplication opcode
                let mult1 = parseParameter(1, input, i, instruction[1], relativeBase);
                let mult2 = parseParameter(2, input, i, instruction[2], relativeBase);
                if (instruction[3] === 0) {
            ...
            case 7: // less-than opcode
                let ltNum1 = parseParameter(1, input, i, instruction[1], relativeBase);
                let ltNum2 = parseParameter(2, input, i, instruction[2], relativeBase);
                if (instruction[3] === 0) {
            ...
            case 8: // equals opcode
                let eqNum1 = parseParameter(1, input, i, instruction[1], relativeBase);
                let eqNum2 = parseParameter(2, input, i, instruction[2], relativeBase);
                if (instruction[3] === 0) {
    ...
    if (isRunning && !notPaused) {
        let programMemory = new ProgramMemory(JSON.parse(JSON.stringify(input)), i, opcodeOutputs);
...
class ProgramMemory {
    constructor(program, i, previousOutput) {
        this.program = program;
        this.i = i;
        this.previousOutput = previousOutput;
    }
}
exports.ProgramMemory = ProgramMemory;
